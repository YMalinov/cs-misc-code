...
class SlotMachine
{
    static void Main()
    {
        ...
        text = "SLOT MACHINE (low performance)";
        ...
        //the difficulty is determined by the amount of characters in availableChars[]
        ...
        //initializing the three variables for the slots
        char firstSlot = new char();
        char secondSlot = new char();
        char thirdSlot = new char();
        ...
        while (true)
        {
            ...
            firstSlot = availableChars[Randomizer.Next(0, availableChars.Length)];
            secondSlot = availableChars[Randomizer.Next(0, availableChars.Length)];
            thirdSlot = availableChars[Randomizer.Next(0, availableChars.Length)];

            Console.ForegroundColor = ConsoleColor.Red;
            Console.SetCursorPosition(Console.BufferWidth / 2 - 2, Console.BufferHeight / 2);
            WriteCharacters(firstSlot, secondSlot, thirdSlot);
            ...
            if (Console.KeyAvailable || stopLoop)
            {
                ...
                if (pressedKey.Key == ConsoleKey.Enter || stopLoop)
                {
                    ...
                    if (lineY == Console.BufferHeight / 2 || stopLoop)
                    {
                        ...
                        if ((firstSlot == jackpot) && (secondSlot == jackpot) && (thirdSlot == jackpot))
                        {
                            text = "      JACKPOT!      ";
            ...
            //of the cycle, won't work with INSANE mode, as INSANE mode is 
            //already hard enough
            if (subtractScore % (difficulty * 70) == 0 && difficulty != 12)
            {
                gameScore--;
            }
            subtractScore += 1;
    ...
    static void WriteCharacters(char firstSlot, char secondSlot, char thirdSlot)
    {
        for (int l = Console.BufferHeight / 2 - 1, i = 0; l <= Console.BufferHeight / 2 + 2; l++, i++)
        {
            Console.SetCursorPosition(Console.BufferWidth / 2 - 7, l);
            Console.Write("{0}", new string(firstSlot, 4));
        }

        for (int l = Console.BufferHeight / 2 - 1, i = 0; l <= Console.BufferHeight / 2 + 2; l++, i++)
        {
            Console.SetCursorPosition(Console.BufferWidth / 2 - 2, l);
            Console.Write("{0}", new string(secondSlot, 4));
        }

        for (int l = Console.BufferHeight / 2 - 1, i = 0; l <= Console.BufferHeight / 2 + 2; l++, i++)
        {
            Console.SetCursorPosition(Console.BufferWidth / 2 + 3, l);
            Console.Write("{0}", new string(thirdSlot, 4));
        }
    }
}